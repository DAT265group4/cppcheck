...
void CheckUncommentedMethod::parseTokens(const Tokenizer &tokenizer, const char FileName[], const Settings *settings)
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        ...
        UncommentedMethod &comments = mFunctions[func->name()];

        if (!comments.lineNumber)
            comments.lineNumber = func->token->linenr();

        // No filename set yet, so we set the fileName
    ...
    for (const Token *tok = tokenizer.tokens(); tok; tok = tok->next()) {
            ...
            while ((scope || start) && markupVarToken) {
                if (markupVarToken->str() == settings->library.blockstart(FileName)) {
                    ...
                    if (mFunctions.find(markupVarToken->str()) != mFunctions.end())
                        mFunctions[markupVarToken->str()].hascomments = true;
                    else if (markupVarToken->next()->str() == "(") {
                        UncommentedMethod &func = mFunctions[markupVarToken->str()];
        ...
        if (funcname) {
            UncommentedMethod &func = mFunctions[funcname->str()];
...
{
    bool errors = false;
    for (std::map<std::string, UncommentedMethod>::const_iterator it = mFunctions.begin(); it != mFunctions.end(); ++it) {
        const UncommentedMethod &func = it->second;
        if (func.hascomments || func.filename.empty())
            continue;
        if (it->first == "main" ||
        ...
        if (!func.hascomments) {
            ...
            uncommentedFunctionError(errorLogger, filename, func.lineNumber, it->first);
            errors = true;
        } else if (!func.hascomments) {
...
void CheckUncommentedMethod::uncommentedFunctionError(ErrorLogger * const errorLogger,
...
Check::FileInfo *CheckUncommentedMethod::getFileInfo(const Tokenizer *tokenizer, const Settings *settings) const
{
...
bool CheckUncommentedMethod::analyseWholeProgram(const CTU::FileInfo *ctu, const std::list<Check::FileInfo*> &fileInfo, const Settings& settings, ErrorLogger &errorLogger)
{
    (void)ctu;
    (void)fileInfo;
...
CheckUncommentedMethod::FunctionDecl::FunctionDecl(const Function *f)
        : functionName(f->name()), lineNumber(f->token->linenr())
{}

std::string CheckUncommentedMethod::analyzerInfo() const
{
...
void CheckUncommentedMethod::analyseWholeProgram(ErrorLogger * const errorLogger, const std::string &buildDir)
{
    ...
    while (std::getline(fin, filesTxtLine)) {
        ...
        for (const tinyxml2::XMLElement *e = rootNode->FirstChildElement(); e; e = e->NextSiblingElement()) {
            ...
            if (checkattr == nullptr || std::strcmp(checkattr,"CheckUncommentedMethod") != 0)
                continue;
    ...
    for (std::map<std::string, Location>::const_iterator decl = decls.begin(); decl != decls.end(); ++decl) {
        ...
        if (calls.find(functionName) == calls.end() && !isOperatorFunction(functionName)) {
            ...
            uncommentedFunctionError(errorLogger, loc.fileName, loc.lineNumber, functionName);
        }
    }
}
